...
public class LogOutputCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args.length) {
            default -> {
                sender.sendMessage(Main.prefix + "Usage: /logoutput <output: Console/TextFile/File> <level>");
            ...
            case 1 -> {
                if (!LogOutput.names().contains(args[0])) {
                    sender.sendMessage(Main.prefix + "Usage: /logoutput <output: console/textfile/file> <level: disabled/valuable/normal/detailed/everything>");
                    return true;
                }
